...
$this->load->model('model_app');
    ...
    function add_jurnal(){
        ...
        $data=array(
			...
			'trans'=>$this->model_app->getdata('detail_jurnal a',"left join jurnal b on a.id_jurnal=b.id_jurnal left join akun c on a.id_parent=c.id_parent WHERE a.id_sub_parent='102' AND  mid(a.tgl_transaksi,6,2)='$bulan' group by a.id_jurnal DESC"),
...
$id_trans=$this->input->post('id_trans');
 ...
 if($this->session->userdata('login_status') == TRUE ){
	 ...
	 if ($this->form_validation->run() == FALSE)
		...
		$dataubah2=array(
		'det_transaksi'=>$this->input->post('det_trans'),
		'jumlah_transaksi'=>$this->input->post('jml_trans'),
		);
		
		$this->model_app->update('jurnal','id_jurnal',$id_trans,$dataubah);
		$this->model_app->update('detail_jurnal','id_jurnal',$id_trans,$dataubah2);
	  redirect('jurnal/add_jurnal');
 }
 }
 else
...
function periode_jurnal(){
	...
	if(isset($_POST['btncetak']))
	{
       ...
       //---------JIKA TOMBOL CARI YANG DITEKAN------------------
		...
		$data['kodejurnal'] =$this->model_app->kode();
		$data['trans'] =$this->model_app->getdata('jurnal a',"left join (detail_jurnal b left join akun c on b.id_parent=c.id_parent) on a.id_jurnal=b.id_jurnal where LEFT(a.tgl_jurnal,4)='$thn' AND MID(a.tgl_jurnal,6,2)='$bln' AND b.id_sub_parent='$status' group by a.id_jurnal");
		$tot_hal = $this->model_app->hitung_isi_tabel('jurnal a',"left join (detail_jurnal b left join akun c on b.id_parent=c.id_parent) on a.id_jurnal=b.id_jurnal where LEFT(a.tgl_jurnal,4)='$thn' AND MID(a.tgl_jurnal,6,2)='$bln' AND b.id_sub_parent='$status' group by a.id_jurnal");
...
'p_modal'=>$this->model_app->getdata('jurnal a',"right join detail_jurnal b on a.id_jurnal=b.id_jurnal WHERE a.id_parent='1028' AND LEFT(a.tgl_jurnal,4)='$thn' AND MID(a.tgl_jurnal,6,2)='$bln' group by a.id_jurnal"),
		...
		ob_start();
		$content = $this->load->view('jurnal/laporan_income',$data);
		$content = ob_get_clean();		
		$this->load->library('html2pdf');
		try
		{
			...
			$html2pdf->pdf->SetDisplayMode('fullpage');
			$html2pdf->writeHTML($content, isset($_GET['vuehtml']));
			$html2pdf->Output('laporan Pemasukan.pdf');
		}
		catch(HTML2PDF_exception $e) {
			echo $e;
			exit;
...
'b_pajak'=>$this->model_app->getdata('jurnal a',"right join detail_jurnal b on a.id_jurnal=b.id_jurnal WHERE a.id_parent='1036' AND LEFT(a.tgl_jurnal,4)='$thn' AND MID(a.tgl_jurnal,6,2)='$bln' group by a.id_jurnal"),
		...
		ob_start();
		$content = $this->load->view('jurnal/laporan_expense',$data);
		$content = ob_get_clean();		
		$this->load->library('html2pdf');
		try
		{
			...
			$html2pdf->pdf->SetDisplayMode('fullpage');
			$html2pdf->writeHTML($content, isset($_GET['vuehtml']));
			$html2pdf->Output('laporan pengeluaran.pdf');
		}
		catch(HTML2PDF_exception $e) {
			echo $e;
			exit;
...
if(isset($_POST['btncetak']))
	{
		...
		ob_start();
		$content = $this->load->view('jurnal/laporan_os_cetak',$data);
		$content = ob_get_clean();		
		$this->load->library('html2pdf');
		try
		{
